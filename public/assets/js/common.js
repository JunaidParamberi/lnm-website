/**
*	lightnmedia - Creative Portfolio Agency HTML Template
*	Version: 1.1
*	Author: bslthemes
*	Author URL: http://themeforest.net/user/bslthemes
*	Copyright © lightnmedia by bslthemes. All Rights Reserved.
**/
...
( function( $ ) {
	...
	**/
	Splitting({ by: 'lines' });
	ScrollOut({targets: '[data-lightnmedia-scroll]', once: true});

	$('.word').wrap('<span></span>');
	$('[data-lightnmedia-overlay]').append('<div class="lightnmedia-overlay"></div>');

	/**
		Header Sticky
	**/
	if($('.lightnmedia-header').length) {
	var new_scroll_position = 0;
	var last_scroll_position;
	$(window).on('scroll', function() {
		var header = $('.lightnmedia-header');
	...
	/**
		Header Menu Button
	**/
	$('.lightnmedia-menu-btn').on('click', function(){
		var obj = $(this);
		if(obj.hasClass('btn--active')) {
			$('body').removeClass('lightnmedia--noscroll');
			$('.lightnmedia-header').removeClass('header--active');
			obj.removeClass('btn--active');
			obj.addClass('lightnmedia--notouch');
			obj.parent().find('.lightnmedia-menu-popup').removeClass('menu--ready');
			obj.parent().find('.lightnmedia-menu-container').addClass('lightnmedia--noscroll');
			setTimeout(function(){
				obj.parent().find('.lightnmedia-menu-popup').removeClass('menu--open');
			}, 300);
			setTimeout(function(){
				obj.removeClass('lightnmedia--notouch');
				obj.parent().find('.lightnmedia-menu-popup').removeClass('menu--visible');
			}, 1600);
		}
		else {
			$('body').addClass('lightnmedia--noscroll');
			$('.lightnmedia-header').addClass('header--active');
			obj.addClass('btn--active');
			obj.addClass('lightnmedia--notouch');
			obj.parent().find('.lightnmedia-menu-popup').addClass('menu--visible');
			obj.parent().find('.lightnmedia-menu-popup').addClass('menu--open');
			setTimeout(function(){
				obj.removeClass('lightnmedia--notouch');
				obj.parent().find('.lightnmedia-menu-container').removeClass('lightnmedia--noscroll');
				obj.parent().find('.lightnmedia-menu-popup').addClass('menu--ready');
			}, 600);
		}
		return false;
	});
	...
	/*
		Top Menu Close
	*/
	$('.lightnmedia-menu').on('click', 'a', function(){
		if (!$(this).parent().hasClass('menu-item-has-children')){
			$('.lightnmedia-header .lightnmedia-menu-btn.btn--active').trigger('click');
		}
	});
	...
	*/
	$('.lightnmedia-menu').on('click', '.menu-item-has-children > .icon, .dropdown-link > a', function(){
	...
	*/
	if($('.footer--fixed').length) {
		var footer_fixed = $('.footer--fixed .lightnmedia-footer').height();
	...
	/*
		Intro Style
	*/
	if($('.lightnmedia-intro.intro--black').length) {
		$('.lightnmedia-header').addClass('header--white');
	}
	...
	*/
	$('.js-hero-parallax').each(function() {
		...
		heroParallax.pagepiling({
			...
			sectionSelector: '.lightnmedia-hero-parallax-section',
			afterRender: function() {
				if (heroParallax.find('.lightnmedia-hero-parallax-section.active').hasClass('section--dark')) {
					setDark();
				}
			...
			afterLoad: function (anchorLink, index) {
				var current = heroParallax.find('.lightnmedia-hero-parallax-section.active');
	...
	function setDark() {
		$('.lightnmedia-page').addClass('parallax--dark');
		$('.lightnmedia-header').removeClass('header--white');
		$('.lightnmedia-hero-parallax .lightnmedia-prev').removeClass('nav--white');
		$('.lightnmedia-hero-parallax .lightnmedia-next').removeClass('nav--white');
	}
	function removeDark() {
		$('.lightnmedia-page').removeClass('parallax--dark');
		$('.lightnmedia-header').addClass('header--white');
		$('.lightnmedia-hero-parallax .lightnmedia-prev').addClass('nav--white');
		$('.lightnmedia-hero-parallax .lightnmedia-next').addClass('nav--white');
	}
	...
	*/
	$('.js-hero-slider').each(function() {
		...
		heroMainSwiper.find('.lightnmedia-paginations-container').append('<div class="swiper-nav-active"></div>');
	...
	*/
	$('.js-blog-carousel').each(function() {
		...
		blogSwiper.parent().find('.lightnmedia-paginations-container').append('<div class="swiper-nav-active"></div>');
	...
	*/
	$('.js-history-slider').each(function() {
		...
		hisSwiper.find('.lightnmedia-paginations-container').append('<div class="swiper-nav-active"></div>');
	...
	*/
	$('.lightnmedia-play-btn').on( 'click', function (){
	...
	/*
		- Services Hover
	*/
	$('.lightnmedia-service-grid-item.active--default').each(function() {
		$(this).closest('.lightnmedia-services-grid-fw').hover(function(){
			if ($(this).find('.lightnmedia-service-grid-item').hasClass('active--default')) {
				$(this).find('.lightnmedia-service-grid-item').removeClass('active');
			}
	  },function(){
			if ($(this).find('.lightnmedia-service-grid-item').hasClass('active--default')) {
				$(this).find('.lightnmedia-service-grid-item.active--default').addClass('active');
			}
	  });
	});
	...
	/*
		- Number Text
	*/
	$('.lightnmedia-circle-text .label').each(function() {
	...
	*/
	if ($('.lightnmedia-showcase-items').length) {
		$('.lightnmedia-showcase-items').find('.lightnmedia-showcase-item:first-child').addClass('hover');
	}
	$('.lightnmedia-showcase-item').mouseenter(function(){
		var obj = $(this);
		obj.closest('.lightnmedia-showcase-items').find('.lightnmedia-showcase-item').removeClass('hover');
		obj.addClass('hover');
	});
	...
	*/
	if ($(window).width() > 767) {
		if ($('.lightnmedia-services-showcase').length) {
			$('.lightnmedia-services-showcase').find('.items ul li:first-child').addClass('hover');
		}
		$('.lightnmedia-services-showcase .items ul li .title').mouseenter(function(){
	...
	/**
		- Video
	**/
	$('.lightnmedia-video').on('click', '.play, .image', function(){
		$(this).closest('.lightnmedia-video').addClass('active');
		var iframe = $(this).closest('.lightnmedia-video').find('.js-video-iframe');
	...
	/**
		- Counter
	**/
	$('.lightnmedia-counter').each(function() {
	...
	/*
		- lightnmedia Button
	*/
	$('button.lightnmedia-btn.lightnmedia-hover-btn').each(function() {
		var btn_text = $(this).text();
		$(this).html('<span>'+btn_text+'</span>');
	});

	/*
		- lightnmedia Pagination
	*/
	$('.pager').each(function() {
		...
		btn_next.html('<i></i>');
		btn_prev.html('<i></i>');
		btn_next.addClass('lightnmedia-next lightnmedia-hover-2');
		btn_prev.addClass('lightnmedia-prev lightnmedia-hover-2');
	});

	/*
		- lightnmedia Portfolio
	*/
	var $container = $('.lightnmedia-portfolio-items');
	$container.imagesLoaded(function() {
		$container.isotope({
			itemSelector: '.lightnmedia-portfolio-col',
	...
	/*
		- lightnmedia Filter
	*/
	$('.lightnmedia-filter-nav-active').each(function() {
		$(this).css({'width':$(this).closest('.lightnmedia-filter.filter--default').find('.item--active').parent().width()+6+'px'});
	});
	$('.lightnmedia-filter').on( 'click', 'button', function() {
		var filterValue = $(this).attr('data-filter');
		$container.isotope({ filter: filterValue });
		$(filterValue).find('.image').attr('data-scroll','in');
		$(filterValue).find('.splitting').attr('data-scroll','in');

		$('.lightnmedia-filter button').removeClass('item--active');
		$(this).addClass('item--active');

		var current_pos = $(this).closest('.lightnmedia-filter.filter--default').find('.item--active').parent().position().left;
		var current_width = $(this).closest('.lightnmedia-filter.filter--default').find('.item--active').parent().width();
		$(this).closest('.lightnmedia-filter.filter--default').find('.lightnmedia-filter-nav-active').css({'width':current_width+6+'px'});
		$(this).closest('.lightnmedia-filter.filter--default').find('.lightnmedia-filter-nav-active').css({'left':current_pos-3+'px'});
	});

	/**
		Collapse
	**/
	$('.lightnmedia-collapse-item').on('click', '.lightnmedia-collapse-btn', function(){
		if($(this).closest('.lightnmedia-collapse-item').hasClass('opened')) {
			$(this).closest('.lightnmedia-collapse-item').removeClass('opened');
			$(this).removeClass('active');
			$(this).next().slideUp();
		}
		else {
			$(this).closest('.lightnmedia-collapse-item').addClass('opened');
			$(this).addClass('active');
			$(this).next().slideDown();
		}
	});
